...
class ForexScheduler:
    ...
    def daily_briefing(self):
        ...
        if self.is_simulation_active() and score['score'] >= 70:
            if self.portfolio.buy(usd_krw):
                print('[페이퍼] 매수 기록됨')
        if self.portfolio.position > 0 and score['score'] < 0 and self.is_simulation_active():
            if self.portfolio.sell(usd_krw):
                print('[페이퍼] 매도 기록됨')

        self.portfolio.update_equity(usd_krw if usd_krw else 0)
        portfolio_summary = self.portfolio.stats(usd_krw if usd_krw else 0)

        market_summary = build_market_summary(usd_krw, *self.analyzer.fetch_upbit_price_and_premium(usd_krw) if usd_krw else (None, None))
        send_score_briefing(
            score_info=score,
    ...
    def afternoon_check(self):
        ...
        if self.portfolio.position > 0:
            ...
            if score['action'] == '매도 검토' or (gap_percent is not None and gap_percent > 0.5):
                ...
                if self.is_simulation_active() and self.portfolio.sell(usd_krw):
                    print('[페이퍼] 매도 기록됨')
    ...
    def weekly_report(self):
        if not self.is_simulation_active():
            return
        current_price = self.analyzer.get_latest_price()
        portfolio_summary = self.portfolio.stats(current_price if current_price else 0)
        send_weekly_report(portfolio_summary)
    ...
    def daily_final_report_check(self):
        ...
        if self._today().strftime('%Y-%m-%d') == FINAL_REPORT_DATE:
            current_price = self.analyzer.get_latest_price()
            portfolio_summary = self.portfolio.stats(current_price if current_price else 0)
            send_final_report(portfolio_summary, self.portfolio.history)
